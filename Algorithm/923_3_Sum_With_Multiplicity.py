...
class Solution:
    def threeSumMulti(self, A, target):
        ...
        :rtype: int
        """
        single_sum = collections.defaultdict(int) # track the count of single elements seen so far
        two_sum = collections.defaultdict(int) # track the count of the sum of any two elements seen so far

        N = len(A)
        MOD = 10 ** 9 + 7
        res = 0
        for i in range(N - 1, -1, -1): # why reversed?
            current = A[i]
            res += two_sum[target - current]
            res %= MOD
            for (num, cnt) in single_sum.items():
                two_sum[num + current] += cnt

            single_sum[current] += 1
